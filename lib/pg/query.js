...
module.exports = {
  async addRequest(binId, mongoId, http_method, http_path) {
    const ADD_REQUEST = "INSERT INTO requests (bin_id, mongo_id, http_method, http_path) " + 
                        "VALUES($1, $2, $3, $4)";

    // we'll need to use getBin first before calling this method or have
    // some other way to get the bin_id
    let result = await pgConnect(ADD_REQUEST, binId, mongoId, http_method, http_path);
